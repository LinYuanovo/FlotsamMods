...
智能互济合并（mergePoweredGrids=true 时，主体连完后执行）:
  【实现后修订 2026-09-22：用户实测发现孤立带电磁岛会抢走远端电池（船只续航不涨）。
  现行为=①生长分两阶段，第一阶段只从镇心带电分量向外长，第二阶段其它带电孤岛才吸收
  剩余；②合并阶段先做「镇心分量无条件吸收够得着的其它带电孤岛」（不需缺电×富余），
  再做原缺电×富余互济；③结果提示未入船网的电池数。测试 T14-T16。详见交接报告 §3.7。】
